...
const newCourse = async (req, res) => {
  ...
  try {
    ...
    if (existedCourse) {
      return res
        .status(409)
        .json({ message: "Course already exist please Create new course" });
    }
...
const uploadAssignmentFormAdmin = async (req, res) => {
  try {
    ...
    if (!course) {
      return res
        .status(404)
        .json({ message: "course not found for assignment!" });
    }
...
const fetchAssignmentsById = async (req, res) => {
  try {
    ...
    const assignments = await Course.findById(courseId)
...
const uploadQuizFromAdmin = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send(
        'Failed to upload a quiz from admin! May be "Correct option is required!"'
      );
  }
};
...
const fetchQuizesById = async (req, res) => {
  try {
    ...
    const quizzes = await Course.findById(courseId)
      .populate("quizzes")
      .select("quizzes");
    if (!quizzes) {
...
const submitQuiz = async (req, res) => {
  try {
    ...
    const { course, quizQuestions, quizNo } = req.body;
...
const getAnnouncementsByCourseId = async (req, res) => {
  try {
    const { courseId } = req.params;
    const courseAnnouncements = await Course.findById(courseId)
...
const getTotalAnnouncements = async (_, res) => {
  try {
    ...
    if (!announcements) {
      return res
        .status(404)
        .json({ message: "Total Announcements not found!" });
    }
    return res.status(200).json({
...
const deleteCourseLecture = async (req, res) => {
  try {
    ...
    res.status(500).send("Failed to delete Course Lecture");
  }
};

const fetchSingleQuiz = async (req, res) => {
  try {
    const { quizId } = req.params;
    const { course, quizNo } = req.body;

    const quiz = await SubmitedQuiz.findOne({
 ...
 const fetchAndCompareQuiz = async (req, res) => {
  try {
    const { quizId } = req.params;
    const { studentId, course, quizNo } = req.body;

    // ?  find the submitted Quiz
    const submittedQuiz = await SubmitedQuiz.findOne({
      quizId: new mongoose.Types.ObjectId(quizId),
      studentId,
      course,
      quizNo
    ...
    if (!submittedQuiz) {
      return res.status(404).json({
        success: false,
        message: "Submitted quiz does not found",
      });
    }
    ...
    if (!quiz) {
      return res.status(404).json({
        success: false,
        message: "original quiz does not exist",
      });
    }

    const comparedQuestions = submittedQuiz.quizQuestions.map((submittedQ) => {
      const originalQ = quiz.quizQuestions.find(
        (q) =>  q.question === submittedQ.question
      );
      const isCorrect = submittedQ?.selectedAnswer === originalQ?.correctOption;
    ...
    console.error("compare quiz error",error);
    return res.status(500).json({
      success: false,
      message: "Server error",
    });
