...
const UploadGrades = () => {
	...
	const handleUpload = async () => {
		if (!selectedCourse || !selectedStudent.name || !selectedStudent._id || !title) {
		...
		try {
			...
			if(result.success) {
				setLoading(false);
				toast.success(`Grades uploaded for ${selectedStudent.name} in ${selectedCourse}`);
		...
		<div className="bg-[#F2F3F8] h-full mt-0 md:pt-10 px-3 md:px-7 p-6">
			...
			<div className="bg-[#F8F8F8] my-6 p-3 md:p-6">
				...
				<div className="form-control mb-4">
					...
					>
						...
						{allStudents?.map((student) => (
							<option key={student._id} value={JSON.stringify(student)}>{student.name}</option>
						))}
					</select>
				</div>
...

				{
					loading ? (
						<button
							className="btn btn-success w-full mt-2 text-base cursor-none"
						>
							<div><Loader2 className="h-6 w-6 animate-spin" /></div>
							please wait
						</button>
					) : (
						<button
