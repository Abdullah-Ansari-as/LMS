...
const Login = () => {
	...
	return (
		<div className='w-full grid grid-cols-1 md:grid-cols-2 tracking-tighter'>
			...
			{/* Left Section */}
			<div className="bg-white md:w-auto flex justify-center my-4 md:my-11">
				<form onSubmit={handleSubmit}>
					...
					<div className='my-5 flex flex-col'>
						...
						/>
						{errors && <span className="text:xs md:text-sm text-red-500">{errors.email}</span>}
					</div>
					<div className='my-5 flex flex-col relative'>
						...
						{errors && <span className="text:xs md:text-sm text-red-500">{errors.password}</span>}
					</div>

					<Link to="/forgot-password"> <span className='text-md text-[#5F57C3] hover:underline cursor-pointer'>Forgot Password ?</span></Link>

					<div className='mt-5 md:mt-7 mb-7 md:mb-14'>
						<button type='submit' className='bg-blue-600 hover:bg-blue-700 w-32 flex justify-center mx-auto rounded-4xl text-white px-4 py-2 text-lg cursor-pointer'>Sign In</button>
						<p className='text-sm mt-4'>if Dont't have an account? <Link to="/signup" className='text-sm text-blue-500 hover:underline cursor-pointer'>SignUp</Link></p>
					</div>
			...
			<div className="hidden md:flex md:h-screen relative">
				...
				<div className="absolute inset-0 flex items-center justify-center bg-black/20 flex-col gap-8">
					<h1 className="text-white text-4xl md:text-5xl font-bold text-outline text-center px-6">
						Learning Management System
					</h1>
					<p className='text-outline-2 text-lg text-white mx-5'>" If you think a thing is impossible, you'll make it impossible. (Bruce Lee) "</p>
				</div>
			</div>
